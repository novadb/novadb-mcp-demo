...
export interface CreateObjectsResponse {
  transaction: { id: number; timestamp: string; branchRef: number; user: string; comment: string };
...
export function createCmsClient(api: ApiClient) {
  return {
    ...
    getTypedObjects: (branch: string, type: string, params: {
      ...
      variants?: string;
      languages?: string;
    } = {}) => api.get(`/branches/${branch}/types/${type}/objects`, { ...params, take: params.take ?? 5 }),
    ...
    getComments: (params: {
      ...
      continue?: string;
      take?: number;
    } = {}) => api.get("/comments", { ...params, take: params.take ?? 5 }),
    ...
    getJobs: (branchId: number, params: {
      ...
      continue?: string;
      take?: number;
    } = {}) => api.get("/jobs", { branchId, ...params, take: params.take ?? 5 }),
